...
-- Changed records: We have 2 records (1 for the change & 1 for the just finished closed record). We process compacted only the 2021&2022 records (20 times less data)
DO $$ -- Check and create the type scd_type if it doesn't exist
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'scd_type') THEN
...
WITH 
	...
	changed_records AS ( -- Changed records
		SELECT 	ts.player_name,
				UNNEST(ARRAY[
					ROW(
						ls.scoring_class,
